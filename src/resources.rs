...
pub async fn load_model(
    ...
    for m in obj_materials? {
        let mut diffuse_texture_file = PathBuf::from(file_name);
        diffuse_texture_file.set_file_name(m.diffuse_texture);
        let diffuse_texture_file = diffuse_texture_file.as_path().display().to_string();
